...
ERP 是认证中心。登录页检测到浏览器里有 ERP token（同域 localStorage）时自动调
`POST /api/auth/sso`：后端拿该 token 请求 `ERP_BASE_URL/api/me` 验证（默认 http://cnc-erp:3000，
经 factory-net），按映射建档/登录并签发本系统 JWT。

角色映射（仅首次自动建档时用；已存在的用户沿用本地角色，本地管理员的调整优先）：

| ERP 角色 | 本系统角色 |
|----------|-----------|
| admin（总经理） | GENERAL_MANAGER |
| procurement（采购主管） | PROCUREMENT_MANAGER |
| cnc_manager（CNC主管） | CNC_SUPERVISOR |
| finance（财务） | GENERAL_MANAGER |
| 其他（文员/跟单/编程/外发） | 不开通，回退手动登录 |

SSO 建档的账号密码为随机不可用值，只能走 SSO 进入；要独立密码就用用户管理改。
ERP 不可达/token 过期时登录页静默回退到本系统账号密码登录，互不拖累。
